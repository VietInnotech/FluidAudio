"""Extract embeddings binary and vocab.json for Qwen3-ASR-1.7B from cached files."""
import json
import struct
import shutil
...
cache_dir = Path(
    "/Users/vit/.cache/huggingface/hub/models--Qwen--Qwen3-ASR-1.7B"
...
# Load embed_tokens from the correct shard
idx = json.loads((cache_dir / "model.safetensors.index.json").read_text())
embed_shard = idx["weight_map"]["thinker.model.embed_tokens.weight"]
print(f"embed_tokens in: {embed_shard}")

weights = load_file(str(cache_dir / embed_shard))
embed_weight = weights["thinker.model.embed_tokens.weight"]
print(f"embed_tokens shape: {embed_weight.shape}, dtype: {embed_weight.dtype}")
...
# Copy vocab.json
shutil.copy2(cache_dir / "vocab.json", output_dir / "vocab.json")
